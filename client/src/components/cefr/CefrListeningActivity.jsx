        ...
        {audioReady ? (
          ...
          <div className="py-3 text-center">
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              This listening activity will be available when its audio is ready.
            </p>
          </div>
        )}
      </div>

      <h2 className="mt-6 text-2xl font-black leading-tight tracking-tight text-slate-950 sm:text-3xl">
        ...
        {options.map((option, index) => {
          ...
          return (
            <button
              key={option.id}
              type="button"
              disabled={locked || !audioReady}
              onClick={() => onSelect?.(option.id)}
              className={`flex w-full items-center gap-4 rounded-2xl border px-4 py-4 text-left transition disabled:cursor-not-allowed disabled:opacity-60 sm:px-5 ${classes}`}
            >
              <span
