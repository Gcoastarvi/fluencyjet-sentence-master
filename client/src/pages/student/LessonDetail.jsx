...
export default function LessonDetail() {
  ...
  const [showLearningPathSticky, setShowLearningPathSticky] = useState(false);

  useEffect(() => {
    if (!showLesson1Onboarding) {
      setShowLearningPathSticky(false);
      return undefined;
    }

    const handleScroll = () => {
      setShowLearningPathSticky(window.scrollY > 420);
    };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans text-slate-900">
      ...
      <main className={`mx-auto max-w-3xl px-6 pt-10 ${showLesson1Onboarding ? "pb-32" : ""}`}>
        {showLesson1Onboarding && (
          <section className="mb-8 rounded-[2rem] bg-gradient-to-br from-indigo-600 to-violet-700 p-6 text-white shadow-xl sm:p-8">
            ...
            <p className="mt-3 max-w-2xl text-base font-semibold leading-7 text-indigo-50 sm:text-lg">
              You finished Quick English. Now watch the short lesson video.
            </p>
          </section>
        )}
        ...
        {showLesson1Onboarding && showLearningPathSticky && (
          <div
            data-testid="lesson1-learning-path-sticky"
            className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 px-4 pt-3 backdrop-blur"
            style={{
              paddingBottom:
